...
def crawl_tacl(cluster_id, ws, inventory_catalog, inventory_schema):
    ...
    waiter = ws.jobs.submit(run_name=f'sdk-{time.time_ns()}',
                           tasks=[
                               j.SubmitTask(existing_cluster_id=cluster_id,
                                            python_wheel_task=j.PythonWheelTask("crawler",
                                                                                package_name="databricks-labs-ucx",
                                                                                parameters=[ws,
                                                                                            inventory_catalog,
                                                                                            inventory_schema]
                                                                                ),
