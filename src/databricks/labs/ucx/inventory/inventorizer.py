...
from databricks.sdk.service.ml import ExperimentTag, ModelDatabricks
...
class MlArtifactsInventorizer(BaseInventorizer[InventoryObject]):
    ...
    def _inventorize_experiments(self):
        ...
        for experiment in experiments:
            ...
            if [i for i in experiment.tags if i in experiments_out_of_scope]:
                logger.info(f"Experiment {experiment.name} with id {exp_id} out of scope")
            else:
                logger.info(f"Fetching permissions for experiment {experiment.name} with id {exp_id}")
                permissions = self._ws.permissions.get(
                    request_object_type=RequestObjectType.EXPERIMENTS, request_object_id=exp_id
                )
    ...
    def _inventorize_models(self):
        for model in self._ws.model_registry.list_models():
            ...
            logger.info(f"Fetching permissions for model {model.name} with id {model_id}")
            permissions = self._ws.permissions.get(
                request_object_type=RequestObjectType.REGISTERED_MODELS, request_object_id=model_id
            )
