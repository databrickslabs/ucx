...
def migrate_external_tables_sync(
    ...
    ).migrate_tables(what=What.EXTERNAL_SYNC, acl_strategy=cfg.grant_migration_strategy)
...
def migrate_dbfs_root_delta_tables(
    ...
    TablesMigrate(
        table_crawler, grant_crawler, ws, sql_backend, table_mapping, group_manager, migration_status_refresher
    ).migrate_tables(what=What.DBFS_ROOT_DELTA, acl_strategy=cfg.grant_migration_strategy)
