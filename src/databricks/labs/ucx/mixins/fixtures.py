...
import pytest
from databricks.labs.lsql.backends import StatementExecutionBackend, RuntimeBackend
...
# for testing, to be removed/modified with older fixture
@pytest.fixture
def spark_backend(ws, env_or_skip) -> RuntimeBackend:
    return RuntimeBackend(ws)


# for testing, to be removed/modified with older fixture
@pytest.fixture
def make_spark_catalog(ws, make_random):
    def create(*, catalog_name: str = None, **kwargs):
        if catalog_name is None:
            catalog_name = f"sdk-{make_random(4)}"
        return ws.catalogs.create(catalog_name, **kwargs)

    yield from factory("spark catalog", create, lambda item: ws.catalogs.delete(item.name))

# for testing, to be removed/modified with older fixture
@pytest.fixture
def make_spark_schema(ws, spark_backend, make_random) -> Generator[Callable[..., SchemaInfo], None, None]:
    def create(*, catalog_name: str = "hive_metastore", name: str | None = None) -> SchemaInfo:
        ...
        spark_backend.execute(f"CREATE SCHEMA {full_name} WITH DBPROPERTIES (RemoveAfter={get_test_purge_time()})")
    ...
    def remove(schema_info: SchemaInfo):
        try:
            spark_backend.execute(f"DROP SCHEMA IF EXISTS {schema_info.full_name} CASCADE")
...
def make_spark_table(ws, spark_backend, make_schema, make_random) -> Generator[Callable[..., TableInfo], None, None]:
    def create(  # pylint: disable=too-many-locals,too-many-arguments,too-many-statements
        *,
        ...
        spark_backend.execute(ddl)

        # CTAS AND NON_DELTA does not support TBLPROPERTIES
        if ctas or non_delta:
            spark_backend.execute(alter_table_tbl_properties)
    ...
    def remove(table_info: TableInfo):
        try:
            spark_backend.execute(f"DROP TABLE IF EXISTS {table_info.full_name}")
        except RuntimeError as e:
            if "Cannot drop a view" in str(e):
                spark_backend.execute(f"DROP VIEW IF EXISTS {table_info.full_name}")
