...
class GroupManager:
    ...
    def _set_migration_groups(self, groups_names: list[str]):
        def get_group_info(name: str):
            ...
            if (
                ws_group is None
            ):  # this can happen upon restart of the tool when the workspace-local group has already been deleted
                logger.info(f"Group {name} not found on the workspace level")
                ws_group = self._recreate_ws_group_from_backup(name, backup_group)
    ...
    def _recreate_ws_group_from_backup(self, ws_group_name, backup_group):
        logger.info(f"Recreating group object {ws_group_name} from backup")
        assert ws_group_name == self._get_source_ws_group_name_from_backup(
            backup_group.display_name
        ), "Workspace-local group and backup have non matching names"

        ws_group = copy.deepcopy(backup_group)
        ws_group.display_name = ws_group_name
        ws_group.id = None  # means recreated, not existing in the workspace

        return ws_group

    def _get_source_ws_group_name_from_backup(self, ws_group_display_name):
        return ws_group_display_name.removeprefix(self.config.backup_group_prefix)
    ...
    def _replace_group(self, migration_info: MigrationGroupInfo):
        ...
        if ws_group.id:
            ...
            logger.info("Group has already been deleted, nothing to do")
    ...
    def _get_backup_groups(self) -> list[iam.Group]:
        ...
        backup_groups = [
            ...
            and self._get_source_ws_group_name_from_backup(g.display_name) in ac_group_names
        ]
    ...
    def prepare_groups_in_environment(self):
        ...
        if group_names:
            ...
            ws_group_names = self._add_missing_ws_groups_from_backup(ws_group_names)
            ac_group_names = {_.display_name for _ in self._account_groups}
    ...
    def _add_missing_ws_groups_from_backup(self, ws_group_names):
        """When the tool is restarted some ws groups might have already been deleted and backup groups created.
        The function find groups that were previously deleted but for which backup group exists to make sure
        the migration state can always be recreated and the tool safely restarted.
        """

        all_ws_group_names = ws_group_names
        for backup_group in self._get_backup_groups():
            source_display_name = self._get_source_ws_group_name_from_backup(backup_group.display_name)
            if source_display_name not in all_ws_group_names:
                all_ws_group_names.add(source_display_name)

        return all_ws_group_names
