...
@ucx.command
def migrate_azure_service_principals(w: WorkspaceClient, new_csv_name: str, replace_with_access_connector: bool):
    ...
    which is generated by save_azure_storage_accounts command. Please review the file and delete or comment out (put # at the beginning of the line)
    the Service Principals that you do not want to be migrated. You can edit the original azure_storage_account_info.csv or
    make a copy of it and pass in the name of the updated file.

    Optionally you can also instruct the command to create an Azure Databricks Access Connector with Managed Identity
    that has the same role assignment as the Azure Service Principal. It's recommended to create the UC storage credential
    using the Access Connector instead of the Azure Service Principal. You can set replace_with_access_connector to true
