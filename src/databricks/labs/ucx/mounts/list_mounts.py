...
from databricks.sdk.service.compute import AwsAttributes, AwsAvailability, Language
...
class MountLister:
    ...
    def create_cluster(self, instance_profile):
        ...
        return self._ws.clusters.create(
    ...
    def execute_code(self, cluster_id, instance, code):
        ...
        text_results = (
            self._ws.command_execution.execute(
                cluster_id=cluster_id, context_id=context.id, language=Language.SCALA, command=code
            )
            .result()
            .results.data.split("\n")[0]
        )
