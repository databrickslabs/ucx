...
class TableInfoCrawler(CrawlerBase[TableInfo]):
    ...
    @classmethod
    def for_paths(cls, backend: SqlBackend, schema) -> TableInfoCrawler:
        return TableInfoCrawler(backend, schema, "table_infos_in_paths")

    @classmethod
    def for_queries(cls, backend: SqlBackend, schema) -> TableInfoCrawler:
        return TableInfoCrawler(backend, schema, "table_infos_in_queries")
    ...
    def __init__(self, backend: SqlBackend, schema: str, table: str):
        ...
        """
        super().__init__(backend=backend, catalog="hive_metastore", schema=schema, table=table, klass=TableInfo)

    def dump_all(self, tables: Sequence[TableInfo]):
    ...
    def _try_fetch(self) -> Iterable[TableInfo]:
    ...
    def _crawl(self) -> Iterable[TableInfo]:
        return []
