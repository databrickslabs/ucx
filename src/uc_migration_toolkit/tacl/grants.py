...
class Grant:
    ...
    @staticmethod
    def type_and_key(*, catalog: str = None, database: str = None, table: str = None, view: str = None,
                     any_file: bool = False, anonymous_function: bool = False) -> (str, str):
    ...
    def uc_sql(self):
        ...
        object_type, object_key = self._this_type_and_key()
        hive_to_uc = {("FUNCTION", "SELECT"): self._uc_action("EXECUTE"),
...
class GrantsCrawler(CrawlerBase):
    ...
    def _crawl(self, catalog: str, database: str) -> list[Grant]:
        ...
        tasks = [partial(self._grants, catalog=catalog),
                 partial(self._grants, catalog=catalog, database=database)]
    ...
    def _grants(self, *, catalog: str = False, database: str = None, table: str = None, view: str = None,
                any_file: bool = False, anonymous_function: bool = False) -> Iterator[Grant]:
        on_type, key = Grant.type_and_key(catalog=self._try_valid(catalog),
                                          database=self._try_valid(database),
                                          table=self._try_valid(table),
                                          view=self._try_valid(view),
                                          any_file=any_file,
                                          anonymous_function=anonymous_function)
        object_type_normalization = {'SCHEMA': 'DATABASE',
                                     'CATALOG$': 'CATALOG'}
        for row in self._fetch(f"SHOW GRANTS ON {on_type} {key}"):
            ...
            if on_type != object_type:
                continue
            yield Grant(principal=principal,
