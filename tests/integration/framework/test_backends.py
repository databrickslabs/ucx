...
def test_sql_backend_should_be_backwards_compatible(sql_backend, inventory_schema):
    ...
    sql_backend.save_table(full_name=f"hive_metastore.{inventory_schema}.jobs", rows=old_rows, klass=OldJobInfo)

    new_rows = [
        JobInfo(job_id="996949600090435", job_name="Cleanup", creator=None, success=1, failures="[]"),
    ]
    with pytest.raises(RuntimeError):
        sql_backend.save_table(full_name=f"hive_metastore.{inventory_schema}.jobs", rows=new_rows, klass=JobInfo)
