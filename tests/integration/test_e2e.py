...
from databricks.labs.ucx.utils import safe_get_scope_acls_for_principal
    ...
    if id_attribute == "workspace_objects":
        ...
        for scope in _scopes:
            for base_group, target_group in zip(comparison_base, comparison_target, strict=True):
                base_acl = safe_get_scope_acls_for_principal(ws, scope.name, base_group.display_name)
                target_acl = safe_get_scope_acls_for_principal(ws, scope.name, target_group.display_name)
