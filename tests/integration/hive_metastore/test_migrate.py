...
@retried(on=[NotFound], timeout=timedelta(minutes=5))
def test_revert_migrated_table(ws, sql_backend, inventory_schema, make_schema, make_table, make_catalog): # pylint: disable=too-many-locals
...
@retried(on=[NotFound], timeout=timedelta(minutes=5))
def test_mapping_reverts_table(ws, sql_backend, inventory_schema, make_schema, make_table, make_catalog): # pylint: disable=too-many-locals
    src_schema = make_schema(catalog_name="hive_metastore")
