...
def test_migrate_table_in_mount(
    ...
    src_external_table = runtime_ctx.make_table(
        schema_name=src_schema.name,
        external_delta=f"dbfs:/mnt/{env_or_skip('TEST_MOUNT_NAME')}/{a_dir}/{b_dir}/{table_path}",
        columns=[("1-0`.0-ugly-column", "STRING")],  # Test with column that needs escaping
    )
    table_in_mount_location = f"abfss://things@labsazurethings.dfs.core.windows.net/{a_dir}/{b_dir}/{table_path}"
