...
from databricks.labs.ucx.managers.group import AvailableGroups, GroupManager
...
def test_account_groups_should_not_be_considered():
    ...
    with patch.object(GroupManager, "_list_account_groups", return_value=[account_admins_group]):
        assert GroupManager(client, group_conf)._list_available_groups() == AvailableGroups(
            workspace=[users_group], account=[account_admins_group]
        )


def test_if_only_account_groups_it_should_return_empty_value():
    client = Mock()
    users_group = Group(display_name="analysts", meta=ResourceMeta(resource_type="Group"))
    account_admins_group = Group(display_name="account admins", meta=ResourceMeta(resource_type="Group"))
    client.groups.list.return_value = []
    with patch.object(GroupManager, "_list_account_groups", return_value=[users_group, account_admins_group]):
        group_conf = GroupsConfig(auto=True)
        assert GroupManager(client, group_conf)._list_available_groups() == AvailableGroups(
            workspace=[], account=[users_group, account_admins_group]
        )
...
def test_backup_group_should_be_created_with_name_defined_in_conf():
    ...
    with patch.object(GroupManager, "_list_account_groups", return_value=[]):
        assert (
...
def test_prepare_groups_in_environment_with_one_group_in_conf_should_return_migrationgroupinfo_object():
    ...
    with patch.object(GroupManager, "_list_account_groups", return_value=[acc_de_group]):
...
def test_prepare_groups_in_environment_with_multiple_groups_in_conf_should_return_two_migrationgroupinfo_object():
    ...
    with patch.object(GroupManager, "_list_account_groups", return_value=[acc_de_group, acc_ds_group]):
        manager = GroupManager(client, group_conf)
        manager.prepare_groups_in_environment()
...
def test_delete_backup_groups():
    ...
    with patch.object(GroupManager, "_list_account_groups", return_value=[test_acc_group]):
        manager = GroupManager(client, group_conf)
        manager.prepare_groups_in_environment()
