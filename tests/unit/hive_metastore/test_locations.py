...
def test_mounts_inventory_should_contain_mounts_without_encryption_type():
    ...
    backend = MockBackend()
    instance = Mounts(backend, client, "hive_metastore", "test")
...
def test_external_locations():
    ...
    crawler = ExternalLocations(Mock(), sql_backend, "hive_metastore", "test")
...
def test_save_external_location_mapping_missing_location():
    ...
    location_crawler = ExternalLocations(ws, sbe, "hive_metastore", "test")
...
def test_mount_listing_multiple_folders():
    ...
    mounts = Mounts(backend, client, "hive_metastore", "test")
    results = TablesInMounts(backend, client, "hive_metastore", "test", mounts).snapshot(force_refresh=True)
    assert results == [
...
def test_filtering_irrelevant_paths():
    ...
    mounts = Mounts(backend, client, "hive_metastore", "test")
    crawler = TablesInMounts(
        backend, client, "hive_metastore", "test", mounts, exclude_paths_in_mount=["$_azuretempfolder"]
    )
...
def test_filter_irrelevant_mounts():
    ...
    mounts = Mounts(backend, client, "hive_metastore", "test")
    crawler = TablesInMounts(backend, client, "hive_metastore", "test", mounts, include_mounts=["/mnt/test_mount"])
    results = crawler.snapshot(force_refresh=True)
...
def test_historical_data_should_be_overwritten():
    ...
    mounts = Mounts(backend, client, "hive_metastore", "test")
    TablesInMounts(backend, client, "hive_metastore", "test", mounts).snapshot(force_refresh=True)
...
def test_mount_include_paths():
    ...
    mounts = Mounts(backend, client, "hive_metastore", "test")
    crawler = TablesInMounts(
        backend, client, "hive_metastore", "test", mounts, include_paths_in_mount=["dbfs:/mnt/test_mount/table2/"]
    )
    results = crawler.snapshot(force_refresh=True)
    assert results == [
        Table("hive_metastore", "mounted_test_mount", "table2", "EXTERNAL", "PARQUET", "adls://bucket/table2"),
