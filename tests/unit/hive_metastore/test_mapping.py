...
def test_table_in_mount_mapping():
    ...
    client.workspace.download.return_value = io.BytesIO(
        "workspace_name,catalog_name,src_schema,dst_schema,src_table,dst_table\r\n"
        "fake_ws,cat1,mounted_datalake,paths,abfss://bucket@msft/path/test,path_test\r\n"
        "fake_ws,cat1,schema2,schema2,table2,dest2\r\n".encode("utf8")
    )
    ...
    backend = MockBackend(
        rows={
            'hive_metastore.test.tables': [
                ("hms", "database", "name", "object_type", "table_format", "abfss://bucket@msft/path/test")
            ],
    ...
    TablesInMountsMapping(installation, client, backend).create_tables_in_uc(tables_in_mounts)
    assert (
...
def test_if_table_exists_in_uc_should_be_skipped():
    ...
    assert ["SELECT * FROM hive_metastore.test.tables WHERE NOT STARTSWITH(database, 'mounted_')", 'DESCRIBE TABLE delta.`abfss://bucket@msft/path/test`'] == backend.queries
...
def test_if_table_exists_in_uc_with_different_schemas_should_throw():
    ...
    client.workspace.download.return_value = io.BytesIO(
        "workspace_name,catalog_name,src_schema,dst_schema,src_table,dst_table\r\n"
        "fake_ws,cat1,mounted_datalake,paths,abfss://bucket@msft/path/test,path_test\r\n"
        "fake_ws,cat1,mounted_datalake,paths,abfss://bucket@msft/path/test2,path_test2\r\n"
        "fake_ws,cat1,mounted_datalake,paths,abfss://bucket@msft/path/test3,path_test3\r\n".encode("utf8")
    )
