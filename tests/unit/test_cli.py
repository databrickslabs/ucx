...
def test_migrate_credentials_azure(ws1, acc_client):
    ws1.config.is_azure = True
    ...
    migrate_credentials(ws1, prompts, ctx=ctx, a=acc_client)
    ws1.storage_credentials.list.assert_called()
    azure_resources.storage_accounts.assert_called()


def test_migrate_credentials_aws(ws1, acc_client):
    ws1.config.is_azure = False
    ws1.config.is_aws = True
...
def test_migrate_credentials_raises_runtime_warning_when_hitting_storage_credential_limit(ws1, acc_client):
...
def test_migrate_credentials_limit_aws(ws1, acc_client):
    ws1.config.is_azure = False
    ws1.config.is_aws = True
    aws_resources = create_autospec(AWSResources)
    external_locations = create_autospec(ExternalLocations)
