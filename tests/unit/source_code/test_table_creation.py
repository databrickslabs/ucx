...
DBR_VERSIONS = [  # version as tuple of ints: (major, minor)
    { "version": None, "suppress": False, },
    { "version": (7, 9), "suppress": False, },
    { "version": (8, 0), "suppress": True, },
    { "version": (9, 0), "suppress": True, },
]
...
def lint(
    ...
    return list(DBRv8_0Linter(dbr_version).lint(code))
...
def test_has_format_arg(migration_index, assign):
    ...
    old_code = get_code(assign, f'spark.foo().format("delta").bar().saveAsTable("catalog.db.table", "csv").baz()')
    assert [] == lint(old_code)
...
def test_has_format_kwarg(migration_index, assign):
    ...
    old_code = get_code(assign, f'spark.foo().format("delta").bar().saveAsTable("catalog.db.table", format="csv").baz()')
    assert [] == lint(old_code)
...
def test_has_format_arg_none(migration_index, assign):
    ...
    old_code = get_code(assign, f'spark.foo().bar().saveAsTable("catalog.db.table", format=None).baz()')
    assert [get_advice(assign, "saveAsTable", 31)] == lint(old_code)


@pytest.mark.parametrize("dbr_version", DBR_VERSIONS)
def test_has_format_arg_none(migration_index, dbr_version):
    """ Tests the DBR version cutoff filter """
    old_code = get_code(False, f'spark.foo().bar().table("catalog.db.table").baz()')
