...
def redash_ws():
    ...
    def api_client_do(method: str, path: str, body: dict = None):
        _ = body
        tags_lookup = {
            "1": [Redash.BACKUP_TAG],
            "2": [Redash.MIGRATED_TAG],
            "3": []
        }
        if method == "POST":
            return {}
...
def test_fix_all_dashboards(redash_ws, empty_fixer):
    ...
    redash_ws.api_client.do.assert_has_calls([
        ...
        call('POST', '/api/2.0/preview/sql/queries/123',
             body={'tags': [Redash.BACKUP_TAG]}),
        # tag the migrated query
        call('POST', '/api/2.0/preview/sql/queries/1',
             body={'query': 'SELECT * FROM old.things',
