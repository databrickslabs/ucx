...
for i in range(10):
    ...
    # ucx[table-migrated-to-uc-python:+3:9:+3:48] Table old.things is migrated to brand.new.stuff in Unity Catalog
    # TODO: Implement missing migration warning (on the source argument):
    # #ucx[table-migrated-to-uc-python:+1:0:+1:0] The default format changed in Databricks Runtime 8.0, from Parquet to Delta
    ...
    # ucx[table-migrated-to-uc-python:+1:9:+1:90] Table old.things is migrated to brand.new.stuff in Unity Catalog
    ...
    # ucx[table-migrated-to-uc-python:+1:9:+1:86] Table old.things is migrated to brand.new.stuff in Unity Catalog
    ...
    # ucx[cannot-autofix-table-reference:+1:9:+1:40] Can't migrate 'spark.catalog.createTable(name)' because its table name argument cannot be computed
    df = spark.catalog.createTable(name)
    do_stuff_with(df)
    # ucx[cannot-autofix-table-reference:+1:9:+1:50] Can't migrate 'spark.catalog.createTable(f'boop{stuff}')' because its table name argument cannot be computed
