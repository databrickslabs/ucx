...
for i in range(10):
    ...
    # ucx[table-migrate:+3:9:+3:56] Table old.things is migrated to brand.new.stuff in Unity Catalog
    # TODO: Implement missing migration warning (on the source argument):
    # #ucx[table-migrate:+1:9:+1:0] The default format changed in Databricks Runtime 8.0, from Parquet to Delta
    ...
    # ucx[table-migrate:+1:9:+1:92] Table old.things is migrated to brand.new.stuff in Unity Catalog
    ...
    # ucx[table-migrate:+1:9:+1:94] Table old.things is migrated to brand.new.stuff in Unity Catalog
