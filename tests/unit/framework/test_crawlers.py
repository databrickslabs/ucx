...
def test_runtime_backend_execute(mocker):
    with mock.patch.dict(os.environ, {"DATABRICKS_RUNTIME_VERSION": "14.0"}):
        ...
        runtime_backend._spark.sql.assert_called_with("CREATE TABLE foo")
...
def test_runtime_backend_fetch(mocker):
    with mock.patch.dict(os.environ, {"DATABRICKS_RUNTIME_VERSION": "14.0"}):
        ...
        runtime_backend = RuntimeBackend()
        runtime_backend._spark.sql().collect.return_value = [1, 2, 3]

        result = runtime_backend.fetch("SELECT id FROM range(3)")

        assert [1, 2, 3] == result

        runtime_backend._spark.sql.assert_called_with("SELECT id FROM range(3)")
...
def test_runtime_backend_save_table(mocker):
    with mock.patch.dict(os.environ, {"DATABRICKS_RUNTIME_VERSION": "14.0"}):
        ...
        runtime_backend._spark.createDataFrame.assert_called_with(
            [Foo(first="aaa", second=True), Foo(first="bbb", second=False)],
            "first STRING NOT NULL, second BOOLEAN NOT NULL",
        )
        runtime_backend._spark.createDataFrame().write.saveAsTable.assert_called_with("a.b.c", mode="append")
...
def test_execute(mocker):
    with mock.patch.dict(os.environ, {"DATABRICKS_RUNTIME_VERSION": "14.0"}):
        ...
        with pytest.raises(NotFound):
            runtime_backend.execute(sql_query)
...
def test_fetch(mocker):
    with mock.patch.dict(os.environ, {"DATABRICKS_RUNTIME_VERSION": "14.0"}):
        ...
        with pytest.raises(NotFound):
            runtime_backend.fetch(sql_query)
