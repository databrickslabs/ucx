...
def test_cluster_assessment(mocker):
    sample_clusters = [
        ...
        ClusterDetails(
            ...
            autoscale=AutoScale(min_workers=1, max_workers=6),
            spark_conf={
                "spark.hadoop.fs.azure.account.oauth2.client.id.ggwstdlrs.dfs.core.windows.net": "{{secrets/ggwiebe/sp_app_client_id}}",
    ...
    mock_cluster_policy.as_dict.return_value = {
        "definition": '{\n  "spark_conf.fs.azure.account.auth.type": {\n    "type": "fixed",\n    "value": "OAuth",\n    "hidden": true\n  }'}
    ws.cluster_policies.get.return_value = mock_cluster_policy
...
def test_cluster_assessment_1(mocker):
    sample_clusters1 = [
        ClusterDetails(
    ...
    mock_cluster_policy = mocker.Mock()
    mock_cluster_policy.as_dict.return_value = {
        "definition": '{"node_type_id":{"type":"allowlist","values":["Standard_DS3_v2","Standard_DS4_v2","Standard_DS5_v2","Standard_NC4as_T4_v3"],"defaultValue":"Standard_DS3_v2"}'}
    ws.cluster_policies.get.return_value = mock_cluster_policy
