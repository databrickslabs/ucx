...
def test_performance(
    ...
    for i in range(NB_OF_TEST_WS_OBJECTS):
        ...
        for j in range(NB_OF_TEST_WS_OBJECTS):
            ...
            assign_ws_local_permissions("notebooks", nb_perms, stat.object_id, ws)
            for group, permission in dir_perms.items():
...
def validate_objects(persisted_rows, sql_backend, test_database, test_groups, ws, object_type):
    for pipe_id in sql_backend.fetch(
        ...
        for acl in acls:
            ...
            if acl.group_name not in test_groups:
                continue
            try:
                assert len(acl.all_permissions) == 1
            except AssertionError as e:
                verificationErrors.append(str(e))
            try:
                assert acl.group_name in persisted_rows[obj_id]
            except AssertionError as e:
                verificationErrors.append(str(e))
            try:
                assert acl.all_permissions[0].permission_level.value == persisted_rows[obj_id][acl.group_name]
            except AssertionError as e:
                verificationErrors.append(str(e))
...
def validate_secrets(persisted_rows, sql_backend, test_database, test_groups, ws):
    for pipe_id in sql_backend.fetch(
        ...
        if acl.principal not in test_groups:
            continue
        try:
            assert acl.permission.value == persisted_rows[obj_id][acl.principal]
        except AssertionError as e:
            verificationErrors.append(str(e))
    ...
    for pipe_id in sql_backend.fetch(
        ...
        for acl in acls:
            ...
            try:
                ...
                verificationErrors.append(str(e))
            try:
                assert acl.permission_level.value == persisted_rows[obj_id][acl.group_name]
            except AssertionError as e:
                verificationErrors.append(str(e))
...
def validate_files(persisted_rows, sql_backend, test_database, test_groups, ws, object_type):
    for pipe_id in sql_backend.fetch(
        f"SELECT distinct object_id FROM {test_database.name}.objects where object_type = '{object_type}'"
    ):
        obj_id = pipe_id["object_id"]
