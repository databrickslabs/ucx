...
To begin, install [uv](https://docs.astral.sh/uv/), which is our build and dependency management tool.
...
Run `make dev` to create the virtual environment and install development dependencies.
...
The easiest fix is to remove the environment and recreate it:
...
uv sync --all-extras
Using CPython 3.10.20 interpreter at: /opt/homebrew/opt/python@3.10/bin/python3.10
Creating virtual environment at: .venv
Resolved 62 packages in 0.86ms
Installed 58 packages in 75ms
...
uv run --exact --all-extras pytest -n 4 --timeout 30 --durations 20 --cov src --cov-report=xml tests/unit --cov-fail-under=89
============================= test session starts ==============================
created: 4/4 workers
4 workers [2019 items]
...
Required test coverage of 89% reached. Total coverage: 89.83%
================= 2011 passed, 4 skipped, 4 xfailed in 45.49s ==================
```
