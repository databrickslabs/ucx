...
also backed up in the ucx installation folder, to allow for easy rollback (see [`revert-dbsql-dashboards` command](/docs/commands#revert-dbsql-dashboards)).
...
Ask your Databricks Account admin to run the [`sync-workspace-info` command](/docs/commands#sync-workspace-info) to sync the
workspace information with the UCX installations. Once the workspace information is synced, you can run the
[`create-table-mapping` command](/docs/commands#create-table-mapping) to align your tables with the Unity Catalog.
